...
void Station::nextHighTideMode (Dstr &text_out,
                           Timestamp startTime,
                           Timestamp endTime,
                           Format::Format form) {
    text_out += "Determining next High/Low Tide times...\n";
    TideEventsOrganizer organizer;
    predictTideEvents(startTime, endTime, organizer);
    TideEventsIterator it = organizer.begin();
    ...
    bool foundH = false;
    bool foundL = false;
    while (!(foundH && foundL) && it != organizer.end()) {
        Dstr line;
        TideEvent e = it->second;
        if (e.isHighTide()){
            e.print(line, Mode::plain, form, *this);
            foundH = true;
            text_out += "H: ";
            text_out += line;
            text_out += '\n';
...
void Station::print (Dstr &text_out,
  ...
  case Mode::plain:
    ...
    plainMode (text_out, startTime, endTime, form);
    break;
  case Mode::nextHighTide:
    if (form != Format::text && form != Format::CSV)
      Global::formatBarf(mode, form);
    nextHighTideMode (text_out, startTime, endTime, form);
    break;
